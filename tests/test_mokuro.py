...
@pytest.mark.parametrize('input_dir_name,disable_html', [
    ('test0', True),
    ('test0', False),
    ('test1_webp', True),
])
...
def test_mokuro(input_dir_name,
                ...
                regenerate):
    input_dir, expected_results_dir = _setup_and_run(input_dir_name, disable_ocr, False, disable_html, tmp_path,
                                                     input_data_root, expected_results_root, regenerate)
...
def test_mokuro_zip(input_dir_name, unzip, tmp_path, input_data_root, expected_results_root, regenerate):
    input_dir, expected_results_dir = _setup_and_run(input_dir_name, False, unzip, True, tmp_path, input_data_root,
                                                     expected_results_root, regenerate)
...
def _setup_and_run(input_dir_name,
                   disable_ocr,
                   unzip,
                   disable_html,
                   tmp_path,
